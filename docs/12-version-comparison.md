...
Tag 查詢屬於輔助資訊。若 `git ls-remote --tags` 失敗或 timeout：

- commit、tree 與 ahead / behind 比對仍需正常完成
- `tagsIdentical` 回傳 `null`
- `sourceTagCheckStatus` 或 `targetTagCheckStatus` 回傳 `FAILED`
- `tagCheckMessage` 保存完整錯誤
- UI 顯示 `Tag 無法確認`，不得將整筆結果改成 `CHECK_FAILED`
