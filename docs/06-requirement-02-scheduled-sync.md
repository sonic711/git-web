...
系統必須支援多個廠商專案與其底下多條 rule，並能依設定排程，自動將各專案來源 branch 推送到對應的目標 remote / branch。
...
- 不使用資料庫。
- 排程設定必須保存在可攜式設定檔中。
- 一筆 rule 的同步邏輯沿用需求 01。
- `manualOnly=true` 的 rule 不得進入排程。
...
建議直接內嵌在 `config/settings.json` 的 `projects[*].rules[*]` 內：
...
2. 系統必須立即清除該 rule 既有的 `nextRunAt`
3. 系統必須立即重新載入排程設定，以下一輪 scheduler 計算新的首次觸發時間
4. 其他人複製該設定檔後應取得相同排程規則

## 排程流程

1. Java 服務啟動時載入設定檔。
2. Java 背景排程器定期掃描啟用中的 rule。
3. 若 rule 為 `manualOnly=true`，直接跳過。
4. 若某筆 rule 已到達執行時間，觸發同步。
5. 若該 rule 尚在執行中，跳過本輪，避免重複執行。
