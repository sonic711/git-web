...
當專案與規則數量增加時，使用者可在 Projects 區的篩選列：

1. 輸入關鍵字搜尋專案名稱、規則名稱、來源 branch、目標 branch、Remote Tab 或目標 repo 名稱。
2. 選擇特定 Remote Tab，例如只顯示 `SIT` 或 `UAT` 規則。
3. 選擇規則模式，例如只顯示同步到 Remote 或只下載到本地的規則。
4. 選擇執行方式，例如自動同步或僅手動。
5. 依最後狀態篩選尚未執行、排隊中、執行中、成功、失敗或中斷的規則。
6. 啟用 `只顯示異常`，快速聚焦最後執行失敗或中斷的規則。
7. 查看「符合筆數 / 全部筆數」摘要。
8. 點擊 `清除篩選` 恢復顯示全部規則。
