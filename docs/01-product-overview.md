...
7. 對具有相同同步規格的多個專案，一次執行來源與目標版本比對。
...
- 每次執行 `sync` 或 `download-only` 前，系統都會強制修剪本機來源 tags，讓本機 tag 快取包含來源的 tag 移動與刪除。
- `download-only` 規則可選填下載本地主目錄覆寫；未設定時使用全域本地主目錄。
- 每次同步按鈕只提交一筆 rule，但手動同步應以背景 job 方式執行，不阻塞 UI。
- 同步 branch 時，系統也應將來源 repo 的 tags 一併推送到目標 remote。一般同步只新增不存在的 tags；勾選 `Force Push` 時，目標 tags 會完整鏡像來源，包含移動既有 tag 與刪除來源已不存在的 tag。
- 第一版以單人使用為主。
...
- 批次版本比對需依既有 sync rules 動態分組，不新增重複的群組設定。
- 批次結果需顯示來源與目標的 commit hash、tree 判定及指向各自 branch HEAD 的 tags。
...
13. 依來源 branch、目標 Remote 與目標 branch 批次執行多專案版本比對。
