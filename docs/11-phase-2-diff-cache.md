...
1. 前端呼叫 `GET /api/rules/{id}/diff/commits/{commitId}/files`
2. 後端流程：
...
### `GET /api/rules/{id}/diff/commits/{commitId}/files`
...
- `selectedCommitIds` 可由 UI 依使用者勾選順序傳入，後端會依來源 branch 歷史順序重新排序
- commit-based sync 透過 `git cherry-pick` 套用選取 commit，因此可能因 commit 相依或目標 branch 差異而衝突
- 發生 cherry-pick conflict 時，本次同步失敗，不會 push
...
6. 本階段不顯示實際 patch 內容
7. 若選取 commit 無法 cherry-pick，UI 可顯示明確失敗訊息
