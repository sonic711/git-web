...
- 每次同步前以 `--force --prune-tags` 對齊本機來源 tags；sync mode 以 tag ref 差異同步目標，Force Push 時包含 tag 移動與來源已刪除 tag 的目標刪除
- 支援 `download-only` 模式，只下載並對齊本地來源分支，不建立 target remote，也不 push，且可使用 rule 層級 `downloadWorkspaceRoot` 覆寫下載主目錄
- 驗證 `allowForcePush`
- 驗證 `manualOnly`
- 驗證 `reviewRequired`
